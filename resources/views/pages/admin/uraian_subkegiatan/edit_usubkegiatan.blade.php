...
<div class="herocontent">
    <div class="row justify-content-center">
        <div class="col-md-8 shadow-xl bg-white rounded">
            <form class="px-5 py-5" action="{{ route('uraian_subkegiatan.update', $uraian->id_uraian_subkegiatan )}}" method="post">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-sm-8">
                        <select class="form-select" name="id_program" id="id_program" required>
                            ...
                            @foreach ($program as $row)
                                <option value="{{ $row->id_program }}" {{$uraian->id_program == $row->id_program ? 'selected' : '' }}>{{ $row->program }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-8">
                        <select class="form-select" name="id_kegiatan" id="id_kegiatan" required>
                            ...
                            @foreach ($kegiatan as $row)
                                <option value="{{ $row->id_kegiatan }}" {{$uraian->id_kegiatan == $row->id_kegiatan ? 'selected' : '' }}>{{ $row->kegiatan }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-8">
                        <select class="form-select" name="id_subkegiatan" id="id_subkegiatan" required>
                            ...
                            @foreach ($subkegiatan as $row)
                                <option value="{{ $row->id_subkegiatan }}" {{$uraian->id_subkegiatan == $row->id_subkegiatan ? 'selected' : '' }}>{{ $row->subkegiatan }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row mt-3">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="kode_rekening" value="{{ $uraian->kode_rekening }}">
                    </div>
                </div>
                <div class="form-group row mt-3">
                    <label class="col-sm-4 control-label">Uraian</label>
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="uraian" value="{{ $uraian->uraian }}">
                    </div>
                </div>
                <div class="form-group row mt-3">
                    <label class="col-sm-4 control-label">Pagu</label>
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="pagu" value="{{ $uraian->pagu }}" onkeypress="return event.charCode >= 48 && event.charCode <= 57">
                    </div>
                </div>
