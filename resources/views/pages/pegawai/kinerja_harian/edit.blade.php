		...
		<div class="col-md-8 shadow-xl bg-white rounded">
			...
			<form class="px-5 py-5"
					...
					<div class="col-sm-8">
						<select id="sasaran" class="form-control" name="sasaran_utama_id" required>
							...
							@foreach ($sasaranUtama as $item)
								<option value="{{ $item->id }}" {{ $item->id == $kinerja_pegawai->sasaran_utama_id ? 'selected' : '' }}>{{ $item->sasaran_strategis }}</option>
							@endforeach
						</select>
					</div>
				</div>
				...
				<div class="form-group row mt-3">
					...
					<div class="col-sm-8">
						<input type="text" style="width:100%" name="kinerja_harian" class="form-control"
							id="kinerja_harian" value="{{ $kinerja_pegawai->kinerja_harian }}" required />
					</div>
				</div>
				<div class="form-group row mt-3">
